...
Vertex: Each Vertex is also known as a node. Vertices are connected to eadch other using edges. A vertex can be connected to itself using an edge.
Edge: Edges are pointers that connect the vertices. An edge can be unidirectional or bidirectional. The number of edges in a graqph always depends upon the number of vertices.
...
Notes:
It is possible that any node in a graph is nbot connected to eachother and it would still be considered a graph.
